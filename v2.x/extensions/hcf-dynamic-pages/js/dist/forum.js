/* =========================================================
   ...
   Build: 2.0.0-rc1
   Flarum 2.x / FriendsOfFlarum Pages 2.x
========================================================= */
(function () {
  'use strict';
  ...
  var DEFAULT_FOLDER = 'v2.x/pages/fof-pages';
  var CACHE_TTL = 60000;
  var cache = new Map();
  ...
  function readConfig(marker) {
    return {
      ...
      folder: marker.getAttribute('data-folder') || DEFAULT_FOLDER,
  ...
  function copyScriptAttributes(from, to) {
    Array.prototype.forEach.call(from.attributes || [], function (attribute) {
      if (attribute.name.toLowerCase() !== 'src') to.setAttribute(attribute.name, attribute.value);
    });
  }
    ...
    return new Promise(function (resolve) {
      ...
      if (src) {
        ...
        script.onerror = function () {
          ...
          console.warn('[HCF Dynamic Pages 2.x] Script failed:', resolved);
          resolve();
        };
  ...
  async function inject(body, html) {
    ...
    holder.innerHTML = normalizeRemoteHtml(html).trim();
    ...
    for (var i = 0; i < scripts.length; i += 1) await runScript(scripts[i]);
  }
  ...
  async function processPage(page) {
    ...
    try {
      ...
      if (!document.documentElement.contains(page) || !document.documentElement.contains(body)) return;
      ...
      page.classList.remove('HCFDynamicPages-loading');
      page.classList.add('HCFDynamicPages-fallback');
      console.warn('[HCF Dynamic Pages 2.x] Falling back to FoF HTML for ' + key, error);
    }
  }

  function scan() {
    var pages = document.querySelectorAll('.Pages[data-id][data-slug]');
    for (var i = 0; i < pages.length; i += 1) processPage(pages[i]);
  }
