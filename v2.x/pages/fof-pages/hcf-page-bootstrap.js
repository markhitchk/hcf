(function () {
'use strict';
var BUILD='1.4.3', OWNER='markhitchk', REPO='hcf', BRANCH='main';
var FOLDER='v2.x/pages/fof-pages';
...
var LOGO='https://cdn.jsdelivr.net/gh/'+OWNER+'/'+REPO+'@'+BRANCH+'/v2.x/assets/logos/HTG.svg';
...
var details=[]; if(key)details.push('Route: /p/'+key); if(status)details.push('Upstream HTTP: '+status); q('.hcf-ex span').textContent=details.length?' // '+details.join(' // '):''; q('.hcf-ef span').textContent='Harley\'s Clan Forum // Flarum 2.x // Loader '+BUILD;
...
emit('hcf:fof-page:error',{build:BUILD,id:id,slug:slug,key:key,type:type,code:ref,template:c.tpl,status:status||null});
}
...
function styles(parsed,base){Array.prototype.forEach.call(parsed.querySelectorAll('style'),function(o){var s=doc.createElement('style');s.setAttribute('data-hcf-fof-import-asset',key);s.textContent=(o.textContent||'').replace(/hcf-page-v2\.1\.css\?v=[A-Za-z0-9._-]+/gi,'hcf-page-v2.1.css?v=2.2b1');(doc.head||doc.documentElement).appendChild(s);o.remove();});Array.prototype.forEach.call(parsed.querySelectorAll('link[rel="stylesheet"]'),function(o){var h=resolve(o.getAttribute('href'),base);if(!h)return;if(/\/hcf-page-v2\.1\.css(?:\?|$)/i.test(h))h=CDN+'hcf-page-v2.1.css?v=2.2b1';var l=doc.createElement('link');l.rel='stylesheet';l.href=cdnize(h);l.setAttribute('data-hcf-fof-import-asset',key);(doc.head||doc.documentElement).appendChild(l);o.remove();});}
function runScript(o,base){return new Promise(function(done){var s=doc.createElement('script'),src=o.getAttribute('src');Array.prototype.forEach.call(o.attributes||[],function(a){if(a.name.toLowerCase()!=='src')s.setAttribute(a.name,a.value);});s.setAttribute('data-hcf-fof-import-asset',key);if(!src){s.textContent=o.textContent||'';(doc.head||doc.documentElement).appendChild(s);s.remove();done(true);return;}var r=cdnize(resolve(src,base));if(/\/hcf-page\.js(?:\?|$)/i.test(r))r='https://cdn.jsdelivr.net/gh/markhitchk/hcf@main/v2.x/pages/fof-pages/hcf-page.js?v=1.4.1';if(doc.querySelector('script[src="'+r.replace(/"/g,'\\"')+'"]')){if(/\/hcf-page\.js(?:\?|$)/i.test(r)&&win.HCFPageRuntime&&typeof win.HCFPageRuntime.refresh==='function'){try{win.HCFPageRuntime.refresh();}catch(e){}}done(true);return;}s.src=r;s.async=false;s.onload=function(){done(true);};s.onerror=function(){console.warn('[HCF FoF Bootstrap] Optional page script failed.');done(false);};(doc.head||doc.documentElement).appendChild(s);});}
